...
ON CONFLICT (slug) DO NOTHING;
INSERT INTO projects (slug, nombre, category, estado, sector, descripcion_es, descripcion_en, url, demo_url) VALUES ('barberia', 'Filo Barber Studio', 'belleza', 'desarrollo', 'servicios-personales', 'Barbería y estética masculina con reserva de cita por la web y un agente de WhatsApp que conversa y agenda solo. Anti-solapamiento garantizado en base de datos y panel de gestión móvil.', 'Barbershop and men’s grooming with online web booking and a WhatsApp AI agent that chats and books appointments on its own. Database-level double-booking prevention and a mobile management panel.', 'https://barberia-demo.vercel.app', 'https://barberia-demo.vercel.app')
ON CONFLICT (slug) DO NOTHING;
